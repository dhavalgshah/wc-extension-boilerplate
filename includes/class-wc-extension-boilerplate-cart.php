 ...
 *
 * @class 	WC_Extension_Boilerplate_Cart
 * @version 0.1.0
 * @since   0.1.0
 */
...
class WC_Extension_Boilerplate_Cart {
	...
	public function is_purchasable( $purchasable , $product ) {
		if( WC_Extension_Boilerplate_Helpers::is_product_checkbox( $product->id ) ){
			$purchasable = true;
		} else {
			$purchasable = false;
		}
	...
	public function add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
		...
		if( $number = WC_Extension_Boilerplate_Helpers::get_sample_number( $product_id ) ){
			$cart_item_data['wc-boilerplate-extension-number'] = $number;
		}

		if( $number = WC_Extension_Boilerplate_Helpers::get_sample_textbox( $product_id ) ){
	...
	public function add_cart_item( $cart_item ) {
		...
		if ( WC_Extension_Boilerplate_Helpers::is_product_checkbox( $cart_item['product_id'] ) ) {
